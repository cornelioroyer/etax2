            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('fel.gestionar')
                            @if (! $factura->fel_documento_id && in_array($factura->estado, ['EMITIDA', 'PARCIAL', 'PAGADA']))
                                <form method="POST" action="{{ route('admin.ventas.facturas.emitir-fel', $factura) }}"
                                      onsubmit="return confirm('¿Enviar la factura {{ $factura->numero }} a la DGI como factura electrónica (FEL)? Se consumirá un folio fiscal.')">
                                    @csrf
                                    <button class="inline-flex items-center gap-1.5 rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-500">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" />
                                        </svg>
                                        Emitir electrónica (FEL)
                                    </button>
                                </form>
                            @endif
