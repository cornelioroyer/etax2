        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3 text-sm">
                ...
                @can('taller.gestionar')
                    <a href="{{ route('admin.taller.tecnicos.create', $tallerId ? ['taller_id' => $tallerId] : []) }}"
                        class="rounded-md bg-indigo-600 px-3 py-1.5 text-white hover:bg-indigo-700">+ Nuevo técnico</a>
                @endcan
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <form method="GET" class="flex gap-2 flex-wrap">
                ...
                <x-text-input name="q" type="search" class="w-56" placeholder="Buscar técnico..." :value="$search" />
                <x-primary-button>Buscar</x-primary-button>
                @if ($search || $tallerId)
                    <a href="{{ route('admin.taller.tecnicos.index') }}" class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">Limpiar</a>
                @endif
            </form>
                ...
                <table class="min-w-full text-sm divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right text-xs font-semibold uppercase text-gray-500">Especialidades</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($tecnicos as $tec)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2 font-mono font-semibold">{{ $tec->codigo }}</td>
                                <td class="px-4 py-2 font-medium">{{ $tec->nombre_publico }}</td>
                                <td class="px-4 py-2 text-xs text-gray-600">{{ $tec->taller->nombre }}</td>
                                <td class="px-4 py-2 text-xs text-gray-600">{{ \App\Models\TallerTecnico::TIPOS[$tec->tipo_tecnico] ?? $tec->tipo_tecnico }}</td>
                                <td class="px-4 py-2 text-right font-mono text-xs">{{ $tec->precio_hora ? number_format($tec->precio_hora, 2) : '—' }}</td>
                                <td class="px-4 py-2 text-right">{{ $tec->especialidades_count }}</td>
                        ...
                        @empty
                            <tr><td colspan="8" class="px-4 py-12 text-center text-gray-400">Sin técnicos registrados.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
