        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3 text-sm">
                ...
                @can('taller.gestionar')
                    <a href="{{ route('admin.taller.sintomas.create', $tallerId ? ['taller_id' => $tallerId] : []) }}"
                        class="rounded-md bg-indigo-600 px-3 py-1.5 text-white hover:bg-indigo-700">+ Nuevo síntoma</a>
                @endcan
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <form method="GET" class="flex gap-2 flex-wrap">
                ...
                <x-text-input name="q" type="search" class="w-56" placeholder="Buscar síntoma..." :value="$search" />
                <x-primary-button>Buscar</x-primary-button>
                @if ($search || $tallerId)
                    <a href="{{ route('admin.taller.sintomas.index') }}" class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">Limpiar</a>
                @endif
            </form>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                <table class="min-w-full text-sm divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($sintomas as $s)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 text-right space-x-3">
                                    @can('taller.gestionar')
                                        <a href="{{ route('admin.taller.sintomas.edit', $s) }}" class="text-xs text-gray-600 hover:underline">Editar</a>
                                        <form method="POST" action="{{ route('admin.taller.sintomas.destroy', $s) }}" class="inline"
                        ...
                        @empty
                            <tr><td colspan="6" class="px-4 py-12 text-center text-gray-400">Sin síntomas registrados.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
