<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar sucursal — {{ $sucursal->nombre }}</h2>
            <a href="{{ route('admin.taller.sucursales.index', ['taller_id' => $sucursal->taller_id]) }}" class="text-sm text-gray-600 hover:text-gray-900">← Sucursales</a>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.taller.sucursales.update', $sucursal) }}">
                    @csrf @method('PUT')
                    <div>
                        <x-input-label value="Taller" />
                        <p class="mt-1 text-sm text-gray-700 font-medium">{{ $sucursal->taller->nombre }}</p>
                    </div>
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <x-input-label for="codigo" value="Código *" />
                            <x-text-input id="codigo" name="codigo" type="text" class="mt-1 block w-full"
                                value="{{ old('codigo', $sucursal->codigo) }}" required maxlength="30" />
                        </div>
                        <div>
                            <x-input-label for="nombre" value="Nombre *" />
                            <x-text-input id="nombre" name="nombre" type="text" class="mt-1 block w-full"
                                value="{{ old('nombre', $sucursal->nombre) }}" required maxlength="200" />
                        </div>
                    </div>
                    <div class="mt-4">
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full"
                                value="{{ old('email', $sucursal->email) }}" maxlength="200" />
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex gap-3">
                        ...
                        <a href="{{ route('admin.taller.sucursales.index', ['taller_id' => $sucursal->taller_id]) }}"
