    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.taller.citas.update', $cita) }}" class="space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Taller</label>
                        <p class="text-sm text-gray-600 bg-gray-50 rounded-md px-3 py-2">{{ $cita->taller?->nombre ?? '—' }}</p>
                    </div>
                        ...
                        <div>
                            ...
                            <select name="sucursal_id"
                                ...
                                @foreach ($sucursales as $s)
                                    <option value="{{ $s->id }}" {{ old('sucursal_id', $cita->sucursal_id) == $s->id ? 'selected' : '' }}>{{ $s->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                    @else
                        <input type="hidden" name="sucursal_id" value="{{ $cita->sucursal_id }}">
                    @endif
                    ...
                    @if ($areas->isNotEmpty())
                        <div>
                            ...
                            <select name="area_id"
                                ...
                                @foreach ($areas as $a)
                                    <option value="{{ $a->id }}" {{ old('area_id', $cita->area_id) == $a->id ? 'selected' : '' }}>{{ $a->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                    @else
                        <input type="hidden" name="area_id" value="{{ $cita->area_id }}">
                    @endif
                        ...
                        <div>
                            ...
                            <select name="tecnico_id"
                                ...
                                @foreach ($tecnicos as $t)
                                    <option value="{{ $t->id }}" {{ old('tecnico_id', $cita->tecnico_id) == $t->id ? 'selected' : '' }}>{{ $t->nombre_publico }}</option>
                                @endforeach
                            </select>
                        </div>
                    @else
                        <input type="hidden" name="tecnico_id" value="{{ $cita->tecnico_id }}">
                    @endif
                    ...
                    <div>
                        ...
                        <x-text-input type="number" name="cliente_id" class="w-full" min="1"
                            placeholder="ID numérico del cliente" :value="old('cliente_id', $cita->cliente_id)" />
                    </div>
                    ...
                    <div>
                        ...
                        <x-text-input type="number" name="equipo_id" class="w-full" min="1"
                            placeholder="ID numérico del equipo" :value="old('equipo_id', $cita->equipo_id)" />
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input type="datetime-local" name="fecha_inicio" class="w-full" required
                                :value="old('fecha_inicio', $cita->fecha_inicio?->format('Y-m-d\TH:i'))" />
                        </div>
                        <div>
                            ...
                            <x-text-input type="datetime-local" name="fecha_fin" class="w-full" required
                                :value="old('fecha_fin', $cita->fecha_fin?->format('Y-m-d\TH:i'))" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="motivo" rows="3"
                            ...
                            placeholder="Motivo de la cita...">{{ old('motivo', $cita->motivo) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="estado" required
                            ...
                            @foreach (\App\Models\TallerCita::ESTADOS as $val => $label)
                                <option value="{{ $val }}" {{ old('estado', $cita->estado) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
