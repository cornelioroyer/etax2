<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Facturas por cobrar</h2>
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.cxc.facturas.index', array_merge(request()->query(), ['export' => 'xlsx'])) }}" class="rounded-md border border-green-300 bg-white px-3 py-2 text-sm text-green-700 hover:bg-green-50">Excel</a>
                <a href="{{ route('admin.cxc.facturas.index', array_merge(request()->query(), ['export' => 'pdf'])) }}" class="rounded-md border border-red-300 bg-white px-3 py-2 text-sm text-red-700 hover:bg-red-50">PDF</a>
                @can('cxc.gestionar')
                    <a href="{{ route('admin.cxc.facturas.create') }}"
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="rounded-lg bg-white p-4 shadow-sm sm:flex sm:items-center sm:justify-between">
                <p class="text-sm text-gray-600">Saldo total por cobrar (facturas vigentes)</p>
            ...
            <form method="GET" class="bg-white p-4 shadow-sm sm:rounded-lg">
                ...
                <div class="mt-3 flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.cxc.facturas.index') }}" class="text-sm text-gray-600 hover:text-gray-900">Limpiar</a>
                </div>
            </form>
                            ...
                            @forelse ($facturas as $factura)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 font-medium">
                                        <a href="{{ route('admin.cxc.facturas.show', $factura) }}" class="text-blue-700 hover:underline">{{ $factura->numero }}</a>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-10 text-center text-gray-500">
                                        ...
                                        @can('cxc.gestionar')
                                            <a href="{{ route('admin.cxc.facturas.create') }}" class="text-blue-700 underline">Crear la primera</a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforelse
