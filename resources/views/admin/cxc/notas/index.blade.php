<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Notas de crédito / débito — CxC</h2>
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.cxc.notas.index', array_merge(request()->query(), ['export' => 'xlsx'])) }}" class="rounded-md border border-green-300 bg-white px-3 py-2 text-sm text-green-700 hover:bg-green-50">Excel</a>
                <a href="{{ route('admin.cxc.notas.index', array_merge(request()->query(), ['export' => 'pdf'])) }}" class="rounded-md border border-red-300 bg-white px-3 py-2 text-sm text-red-700 hover:bg-red-50">PDF</a>
                @can('cxc.gestionar')
                    <a href="{{ route('admin.cxc.notas.create', ['tipo' => 'credito']) }}" class="rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">+ Nota de crédito</a>
                    <a href="{{ route('admin.cxc.notas.create', ['tipo' => 'debito']) }}" class="rounded-md bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700">+ Nota de débito</a>
                @endcan
            </div>
        </div>
    </x-slot>
            ...
            <form method="GET" class="bg-white p-4 shadow-sm sm:rounded-lg">
                <div class="flex flex-wrap items-end gap-3">
                    ...
                    <div>
                        ...
                        <select id="cliente_id" name="cliente_id" class="mt-1 block w-64 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @forelse ($notas as $nota)
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 font-medium">
                                        <a href="{{ route('admin.cxc.notas.show', $nota) }}" class="text-blue-700 hover:underline">{{ $nota->numero }}</a>
                                    </td>
                                    <td class="px-4 py-3">
