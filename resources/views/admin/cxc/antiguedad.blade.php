<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Antigüedad de saldos — CxC</h2>
            <div class="flex items-center gap-2 print:hidden">
                <a href="{{ route('admin.cxc.antiguedad', array_merge(request()->query(), ['export' => 'xlsx'])) }}" class="rounded-md border border-green-300 bg-white px-3 py-2 text-sm text-green-700 hover:bg-green-50">Excel</a>
                <a href="{{ route('admin.cxc.antiguedad', array_merge(request()->query(), ['export' => 'pdf'])) }}" class="rounded-md border border-red-300 bg-white px-3 py-2 text-sm text-red-700 hover:bg-red-50">PDF</a>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                <div class="border-b border-gray-100 px-4 py-3 text-sm text-gray-600">
                    Facturas con saldo al {{ $corte->format('d/m/Y') }} — la edad se mide en meses completos desde la fecha de vencimiento.
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($clientes as $fila)
                                <tr x-data="{ abierto: false }" class="hover:bg-gray-50 cursor-pointer" @click="abierto = ! abierto">
                                    <td class="px-4 py-3 font-medium text-gray-900">{{ $fila['cliente']->nombre ?? '—' }}</td>
                                    @foreach ($columnas as $clave => $titulo)
                                ...
                                @foreach ($fila['facturas'] as $detalle)
                                    <tr class="bg-gray-50/60 text-xs text-gray-600">
                                        <td class="px-4 py-1.5 pl-8">
                                            <a href="{{ route('admin.cxc.facturas.show', $detalle['doc']) }}" class="text-blue-700 hover:underline">{{ $detalle['doc']->numero }}</a>
