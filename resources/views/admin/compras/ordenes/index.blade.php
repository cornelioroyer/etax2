    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            @can('compras.gestionar')
                <a href="{{ route('admin.compras.ordenes.create') }}"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <form method="GET" class="bg-white p-4 shadow-sm sm:rounded-lg">
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-6">
                    ...
                    <div>
                        ...
                        <select id="estado" name="estado" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach (['BORRADOR', 'APROBADA', 'RECIBIDA_PARCIAL', 'RECIBIDA', 'FACTURADA', 'ANULADA'] as $estado)
                                <option value="{{ $estado }}" @selected(($filtros['estado'] ?? '') === $estado)>{{ ucfirst(strtolower(str_replace('_', ' ', $estado))) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div class="mt-3 flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.compras.ordenes.index') }}" class="text-sm text-gray-600 hover:text-gray-900">Limpiar</a>
                </div>
            </form>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($ordenes as $orden)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">{{ $orden->fecha->format('d/m/Y') }}</td>
                                    <td class="px-4 py-3 max-w-xs truncate">{{ $orden->proveedor->nombre ?? '—' }}</td>
                                    <td class="px-4 py-3 text-right whitespace-nowrap font-medium">B/. {{ number_format((float) $orden->total, 2) }}</td>
                                    <td class="px-4 py-3">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-10 text-center text-gray-500">
                                        ...
                                        @can('compras.gestionar')
                                            <a href="{{ route('admin.compras.ordenes.create') }}" class="text-blue-700 underline">Crear la primera</a>
                                        @endcan
                                    </td>
                                </tr>
                            @endforelse
                ...
                @if ($ordenes->hasPages())
                    <div class="border-t border-gray-100 px-4 py-3">{{ $ordenes->links() }}</div>
                @endif
            </div>
        </div>
    </div>
