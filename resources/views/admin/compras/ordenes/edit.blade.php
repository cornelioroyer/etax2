<x-app-layout>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <x-buscador-contacto name="proveedor_id" label="Proveedor *" required
                                ...
                                :opciones="$proveedores" :selected="old('proveedor_id') ?? $orden->proveedor_id" />
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-sm">
                                <thead class="text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                                    <tr>
                                        <th class="py-2 pr-2 min-w-[16rem]">Descripción / Artículo</th>
                                        <th class="w-24 py-2 pr-2 text-right">Cant.</th>
                                        <th class="w-28 py-2 pr-2 text-right">Precio</th>
                                        <th class="w-28 py-2 pr-2">ITBMS</th>
                                        <th class="w-48 py-2 pr-2">Cuenta contable</th>
                                            ...
                                            <td class="py-2 pr-2">
                                                ...
                                                <div class="relative">
                                                    <input type="text"
                                                           ...
                                                           @input="linea.mostrarItems = true; linea.item_id = null"
                                                           @focus="linea.mostrarItems = true"
                                                           @keydown.escape="linea.mostrarItems = false"
                                                           required
                                                           placeholder="Descripción o buscar artículo…"
                                                           class="block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                    <div x-show="linea.mostrarItems && linea.descripcion.length > 0"
                                                         @click.outside="linea.mostrarItems = false"
                                                         class="absolute z-10 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg text-sm">
                                                        <template x-for="item in itemsFiltrados(linea.descripcion)" :key="item.id">
                                                            <div class="cursor-pointer px-3 py-2 hover:bg-indigo-50"
                                                                 @mousedown.prevent="seleccionarItem(linea, item)">
                                                                <span class="font-mono text-gray-400 text-xs" x-text="item.codigo + ' '"></span>
                                                                <span x-text="item.nombre"></span>
                                                            </div>
                                                        </template>
                                                        <div x-show="itemsFiltrados(linea.descripcion).length === 0"
                                                             class="px-3 py-2 text-gray-400">Sin coincidencias — escribe la descripción libre</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="py-2 pr-2">
    ...
    <script>
        function ordenForm(lineasIniciales, impuestos, items) {
            const impuestoDefault = impuestos.find(i => i.porcentaje == 7)?.id ?? impuestos[0]?.id ?? null;
            const nueva = () => ({ item_id: null, descripcion: '', cantidad: 1, precio_unitario: 0, impuesto_id: impuestoDefault, cuenta_id: '', mostrarItems: false });
            ...
            return {
                ...
                lineas: lineasIniciales.length
                    ? lineasIniciales.map(l => ({
                        ...
                        cuenta_id: l.cuenta_id ? parseInt(l.cuenta_id) : '',
                        mostrarItems: false,
                    }))
                    : [nueva()],
                ...
                itemsFiltrados(busqueda) {
                    ...
                    return this.items.filter(i =>
                        i.nombre.toLowerCase().includes(b) || (i.codigo && i.codigo.toLowerCase().includes(b))
                    ).slice(0, 10);
                },
