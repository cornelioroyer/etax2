<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar borrador {{ $factura->numero }}</h2>
            <a href="{{ route('admin.cxp.facturas.show', $factura) }}" class="text-sm text-gray-600 hover:text-gray-900">← Volver</a>
        </div>
    </x-slot>

    @php
        $lineasJson = $factura->detalle->map(function ($l) {
            $bruto = round((float) $l->cantidad * (float) $l->precio_unitario, 2);
            // El ITBMS se calcula sobre la base NETA (bruto − descuento), así que la tasa
            // se recupera dividiendo el impuesto entre esa base neta, no entre el bruto.
            $baseNeta = round($bruto - (float) $l->descuento, 2);
            $tasa = $baseNeta > 0 ? (int) round((float) $l->impuesto_monto / $baseNeta * 100) : 0;
            return [
                'item_id' => $l->item_id,
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.cxp.facturas.update', $factura) }}"
                      x-data="facturaCxp({{ old('lineas') ? collect(old('lineas'))->values()->toJson() : $lineasJson }}, {{ (int) ($cuentaGastoId ?? 0) }}, {{ $proveedores->pluck('cuenta_gasto_id', 'id')->toJson() }}, {{ (float) old('descuento_general', 0) }})">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="numero" name="numero" type="text" class="mt-1 block w-full" required
                                          :value="old('numero', $factura->numero)" placeholder="La del proveedor" />
                    ...
                    <div class="mt-6 flex flex-wrap items-center gap-3 border-t border-gray-100 pt-4">
                        ...
                        <a href="{{ route('admin.cxp.facturas.show', $factura) }}" class="text-sm text-gray-600 hover:text-gray-900">Cancelar</a>
                        <p class="w-full text-xs text-gray-500 sm:w-auto sm:ml-auto">El asiento contable se genera al contabilizar la factura.</p>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        // Escribe en linea.cuenta_id (reactivo de Alpine), sin select aparte.
        function comboCuenta(linea) {
            return {
                ...
                init() {
                    this.sync();
                    // Si la cuenta cambia por fuera (al cambiar proveedor), resincroniza el texto.
    ...
    <script>
        ...
        function facturaCxp(lineasIniciales, cuentaGastoId, provCuentas, descuentoGeneralInicial) {
            return {
                ...
                lineas: lineasIniciales.length
                    ...
                    : [{ item_id: null, busqueda: '', mostrarItems: false, descripcion: '', cantidad: 1, precio_unitario: 0, descuento: 0, tasa_itbms: 7, cuenta_id: cuentaGastoId || '' }],
                ...
                onProveedor(provId) {
                    ...
                    // Solo afecta líneas vacías o que tenían la cuenta por defecto anterior (no las editadas a mano)
