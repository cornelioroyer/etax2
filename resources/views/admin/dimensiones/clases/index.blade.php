<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Clases de asiento</h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <h3 class="mb-3 text-sm font-semibold text-gray-700">Nueva clase</h3>
                <form method="POST" action="{{ route('admin.dimensiones.clases.store') }}">
                    @csrf
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <x-input-label for="codigo" value="Código *" />
                            <x-text-input id="codigo" name="codigo" type="text" class="mt-1 block w-full"
                                :value="old('codigo')" placeholder="OPE" required maxlength="30" />
                        </div>
                        <div>
                    ...
                    <div class="mt-4">
                        <x-primary-button>Guardar clase</x-primary-button>
                    </div>
                </form>
            </div>
            @endcan
                        ...
                        @forelse ($items as $item)
                            <tr x-data="{ edit: false }" class="{{ $item->activo ? '' : 'opacity-50' }}">
                                ...
                                <td class="px-4 py-2 text-right space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.dimensiones.clases.destroy', $item) }}" class="inline"
                            ...
                            <tr x-show="edit" x-cloak>
                                <td colspan="4" class="bg-gray-50 px-4 py-3">
                                    <form method="POST" action="{{ route('admin.dimensiones.clases.update', $item) }}">
                                        @csrf @method('PUT')
                            ...
                            @endcan
                        @empty
                            <tr><td colspan="4" class="px-4 py-6 text-center text-gray-400">Sin clases registradas.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
