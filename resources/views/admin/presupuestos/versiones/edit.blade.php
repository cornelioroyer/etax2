<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar versión</h2>
    ...
    <div class="py-8">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('admin.presupuestos.versiones.update', $version) }}">
                    @csrf @method('PUT')
                    <div>
                        <x-input-label for="nombre" value="Nombre de la versión *" />
                        <x-text-input id="nombre" name="nombre" type="text" class="mt-1 block w-full"
                            :value="old('nombre', $version->nombre)" required maxlength="100" />
                    </div>
                    <label class="mt-4 flex items-center gap-2 text-sm text-gray-700">
                        <input type="hidden" name="activa" value="0">
                        <input type="checkbox" name="activa" value="1" {{ old('activa', $version->activa) ? 'checked' : '' }}
