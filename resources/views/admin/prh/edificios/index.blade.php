    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3 text-sm">
                <a href="{{ route('admin.prh.propietarios.index') }}" class="text-gray-500 hover:text-gray-900">Propietarios</a>
                <a href="{{ route('admin.prh.tipos-cuota.index') }}" class="text-gray-500 hover:text-gray-900">Tipos de cuota</a>
                <a href="{{ route('admin.prh.cuotas.index') }}" class="text-gray-500 hover:text-gray-900">Cuotas</a>
                <a href="{{ route('admin.prh.pagos.index') }}" class="text-gray-500 hover:text-gray-900">Pagos</a>
                @can('prh.gestionar')
                    <a href="{{ route('admin.prh.edificios.create') }}"
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <form method="GET" class="flex gap-2">
                ...
                @if ($search)
                    <a href="{{ route('admin.prh.edificios.index') }}" class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">Limpiar</a>
                @endif
            </form>
                        ...
                        @forelse ($edificios as $ed)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 text-right space-x-3">
                                    <a href="{{ route('admin.prh.edificios.show', $ed) }}" class="text-xs text-indigo-600 hover:underline">Ver</a>
                                    @can('prh.gestionar')
                                        <a href="{{ route('admin.prh.edificios.edit', $ed) }}" class="text-xs text-gray-600 hover:underline">Editar</a>
                                    @endcan
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-12 text-center text-gray-400">
                                    ...
                                    @can('prh.gestionar')
                                        <a href="{{ route('admin.prh.edificios.create') }}" class="text-indigo-600 hover:underline">Crear el primero</a>
                                    @endcan
                                </td>
                            </tr>
                        @endforelse
