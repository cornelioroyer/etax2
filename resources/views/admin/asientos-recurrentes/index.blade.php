<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Asientos recurrentes</h2>
            @can('contabilidad.crear')
                <a href="{{ route('admin.asientos-recurrentes.create') }}"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            @if ($pendientes > 0)
                <div class="flex flex-wrap items-center justify-between gap-3 rounded-md bg-amber-50 p-4 text-sm text-amber-800">
                    <span>Hay <strong>{{ $pendientes }}</strong> plantilla(s) con vencimientos pendientes hasta hoy. El sistema los genera solo a diario, pero puedes generarlos ahora.</span>
                    @can('contabilidad.crear')
                        <form method="POST" action="{{ route('admin.asientos-recurrentes.generar-todos') }}">
                            @csrf
            ...
            <form method="GET" class="flex flex-wrap items-end gap-3 rounded-md bg-white p-4 shadow-sm">
                ...
                <div>
                    ...
                    <input type="text" name="q" value="{{ $filtros['q'] ?? '' }}" placeholder="Nombre o concepto"
                ...
                @if (($filtros['estado'] ?? '') || ($filtros['q'] ?? ''))
                    <a href="{{ route('admin.asientos-recurrentes.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Limpiar</a>
                @endif
            </form>
                ...
                <table class="min-w-full text-sm">
                    <thead class="border-b border-gray-200 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                        <tr>
                            ...
                            <th class="px-4 py-3">Generados</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($plantillas as $p)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3">
                                    <a href="{{ route('admin.asientos-recurrentes.show', $p) }}" class="font-medium text-blue-700 hover:underline">{{ $p->nombre }}</a>
                                ...
                                <td class="px-4 py-3 text-right text-gray-700">B/. {{ number_format($p->total_debito, 2) }}</td>
                                <td class="px-4 py-3 text-gray-600">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-end gap-2">
                                        @can('contabilidad.editar')
                                            @if ($p->esActiva())
                                                <form method="POST" action="{{ route('admin.asientos-recurrentes.pausar', $p) }}">@csrf
                                                    <button class="text-xs text-gray-500 hover:text-gray-700">Pausar</button>
                                                </form>
                                            @elseif (! $p->estaFinalizada())
                                                <form method="POST" action="{{ route('admin.asientos-recurrentes.reactivar', $p) }}">@csrf
                                                    <button class="text-xs text-green-600 hover:text-green-800">Reactivar</button>
                                                </form>
                                            @endif
                                        @endcan
                                        <a href="{{ route('admin.asientos-recurrentes.show', $p) }}" class="text-xs text-blue-600 hover:underline">Ver</a>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">No hay plantillas recurrentes. Crea la primera con «Nueva plantilla».</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
