<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Planilla — Departamentos</h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <h3 class="mb-3 text-sm font-semibold text-gray-700">Nuevo departamento</h3>
                <form method="POST" action="{{ route('admin.nomina.departamentos.store') }}">
                    @csrf
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <x-input-label for="codigo" value="Código *" />
                            <x-text-input id="codigo" name="codigo" type="text" class="mt-1 block w-full"
                                :value="old('codigo')" placeholder="ADM" required maxlength="20" />
                        </div>
                        <div>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                <table class="min-w-full text-sm divide-y divide-gray-200">
                    ...
                    @forelse ($items as $item)
                        <tbody x-data="{ edit: false }" class="divide-y divide-gray-100">
                            <tr class="{{ $item->activo ? '' : 'opacity-50' }}">
                                ...
                                <td class="px-4 py-2 text-center">{{ $item->empleados_count }}</td>
                                ...
                                <td class="px-4 py-2 text-right space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.nomina.departamentos.destroy', $item) }}" class="inline"
                    ...
                    @empty
                        <tbody><tr><td colspan="5" class="px-4 py-6 text-center text-gray-400">Sin departamentos registrados.</td></tr></tbody>
                    @endforelse
                </table>
            </div>
        </div>
